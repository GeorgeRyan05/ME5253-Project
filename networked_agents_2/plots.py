...
def globally_averaged_plot(centralized_J, distributed_J, results_path=None):
    ...
    plt.plot(
        X,
        np.mean(np.array(centralized_J), axis=0),
...
def pi_plot(centralized_pis, decentralized_pis, results_path=None):
    ...
    rects1 = ax.bar(
        X - width / 2,
        centralized_pis.tolist(),
        width,
        color=CENTRALIZED_AGENT_COLOR,
        label="Centralized",
    )
    rects2 = ax.bar(
        X + width / 2,
        decentralized_pis.tolist(),
        width,
...
def log_plot(centralized_log, distributed_log, results_path=None):
    ...
    best_actions, best_actions_rewards = (
        centralized_log["best_actions"],
        centralized_log["best_actions_rewards"],
    )
    ...
    centralized_actions, distributed_actions = (
        centralized_log["actions"],
        distributed_log["actions"],
    )
    actions_plot(centralized_actions, distributed_actions, results_path)
    # rewards plot
    centralized_average_rewards, distributed_average_rewards = (
        centralized_log["reward"],
        distributed_log["reward"],
    )
    average_rewards_plot(
        centralized_average_rewards, distributed_average_rewards, results_path
    )
...
def states_plot(centralized_states, distributed_states, results_path):
    ...
    plt.plot(
        X, smoothed_T[:, 1], label="Smoothed Centralized", c=CENTRALIZED_AGENT_COLOR
    )
