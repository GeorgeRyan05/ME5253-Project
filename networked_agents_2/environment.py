    ...
    def __init__(
        ...
        for n_dim in range(n_dims):
            ...
            for i, digit in enumerate(digits):
                self.R[n_dim, i] = (
                    4 * ((n_state + 1) - (1 - int(digit == "1"))) / self.n_states
                )
        ...
        # [n_states, n_actions, n_nodes, n_varphi]
        self.VARPHI = np.tile(
            uniform(size=(n_states, n_actions, 1, n_varphi)), (1, 1, n_nodes, 1)
        )
...
if __name__ == "__main__":
    ...
    np.testing.assert_almost_equal(
        phi, env.PHI[env.state * env.n_action_space + bin2dec(actions), :]
    )
